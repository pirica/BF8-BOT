...
module.exports = {
    ...
    async execute(message, args, bot, prefix) {
        ...
        axios({
            ...
            let text = "انقر على رد الفعل المقابل للإحصائيات التي تريد عرضها :\n\n<:global:744916178081939477> Toutes les plateformes"
            ...
            embed.setTitle("حدد النظام الأساسي الخاص بك")
            .setDescription(text)
            .setColor("#0078ff")
            .setFooter(`${message.author.username}`, message.author.displayAvatarURL({dynamic: true}))
            .setTimestamp()
            ...
            collector.on('end', async (collected) => {
                ...
                if(collected.size == 0) {
                    ...
                    if(stats.overall !== null) {
                        ...
                        ctx.font = 'italic 40px Burbank Big Rg Bd'
                        ctx.fillText("لم يتم العثور على إحصائيات", 680, 400)
                    }
            ...
            if(e.response.status == 404) {
                ...
                .setDescription(":warning: تحقق من اسم الحساب !")
                .setColor("#cf3419")
                .setFooter(message.author.username, message.author.displayAvatarURL({dynamic: true}))
                .setTimestamp()
                return message.channel.send(embed)
            } else if(e.response.status == 403) {
                embed.setTitle("إحصائيات الحساب الخاص !")
                .setDescription(":warning: الإحصائيات الخاصة بك في سرية !\nهذه هي الحيلة لجعلها في الأماكن العامة ! ^^")
                ...
                .setDescription(`\`${e.resonse.data.error}\`\nحاول مرة أخرى واتصل بهورزيوكس إذا استمرت المشكلة!`)
                .setColor("#cf3419")
                .setFooter(message.author.username, message.author.displayAvatarURL({dynamic: true}))
                .setTimestamp()
                return message.channel.send(embed)
